    ...
    const submitPost = async (e) => {
        ...
        if(userImage) {
          const { data, error } = await supabase.storage
                .from('images')
                .upload(`public/${userImage.name}`, userImage);
          ...
          const { publicURL } = supabase.storage    
              .from('images')
              .getPublicUrl(`public/${userImage.name}`);
          imageURL = publicURL
        }
        try {
  ...
  return (
    <div>
      ...
      <div className='flex flex-col items-center justify-center'>
          ...
          <input className='w-[28rem] h-[5rem] rounded-2xl bg-[rgba(129,129,129,0.49)] text-center placeholder:font-mono placeholder:text-lg pr-1' onChange={handleChange} id='title' name='title' type="text" placeholder='enter a title...' />

          <h2 className='text-4xl mt-10'>Additional Info</h2>
          <input className='w-[491px] h-[223px] bg-[rgba(124,54,54,0.49)] mt-7 mb-7 text-left placeholder:font-mono placeholder:text-lg pl-2 pr-1 pb-[10rem]' onChange={handleChange} type="text" id='details' name='details' placeholder='any additional info...' />
